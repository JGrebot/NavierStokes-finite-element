...
double g(const R2 & P, int label,int c)
{
 ...
 int ccount=0;
const int lastit=250;       // <------------------ NOMBRE D'ITERATION <!>
const double dt=0.1;
//const double T=dt*lastit;
...
int  main(int argc, const char** argv)
{
  ...
  double errLinf=normsum(&u[0],&uold[0],n2+n2);


  cout << " Erreur Récursive L2 = "<<  " " << errL2  << endl;
  cout << " Erreur Récursive Linf = "<<  " " << errLinf  << endl;
...
if(kkk==lastit){
    ...
    char ff[256];
    sprintf(ff,"FreeFem++ check43.edp -ns -Th %s ",argv[1]);
    system(ff);
    std::cout << "Iteration C++" << kkk << '\n';
    cout << " Erreur Récursive L2 = "<<  " " << errL2  << endl;
    cout << " Erreur Récursive Linf = "<<  " " << errLinf  << endl;
