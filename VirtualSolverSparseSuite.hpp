...
public:
    ...
    void dosolver(K *x,K*b,int N) {assert(0);}
...
public:
    ...
    void dosolver(double *x,double*b,int N) {
...
public:
    ...
    void dosolver(K *x,K*b,int N) {
        for(int k=0,oo=0; k<N;++k, oo+= A->n)
        {
            double * xx = (double *) (void*) x+oo,  *bb = (double *) (void*) b+oo, *zx=0;;
            status= umfpack_zi_solve (UMFPACK_A, Ap, Ai, Ax,Az, xx,zx, bb, zx , Numeric, 0, 0) ;
            if(status) cout << " Error umfpack_di_solve  status  " << status << endl;
        }
